import { SAML, type Profile, type SamlConfig as NodeSamlConfig, ValidateInResponseTo } from '@node-saml/node-saml';
...
function normalizeRedirect(relayState?: string | null) {
  const defaultRedirect = config.samlDefaultRedirect || `${config.appBaseUrl || '/'}/`;
  if (!relayState) return defaultRedirect;
...
async function ensureOrg(orgId: string, name?: string) {
  await pool.query(
...
async function logAudit(orgId: string | null, email: string | null, event: string, details: Record<string, unknown> = {}) {
  try {
    await pool.query(
      `INSERT INTO saml_audit_logs (org_id, user_email, event_type, details)
       VALUES ($1, $2, $3, $4)`,
      [orgId, email, event, JSON.stringify(details)],
    );
  } catch (error) {
...
export async function fetchIdpMetadata(url: string) {
  ...
  const certificate =
    typeof certNode === 'string'
      ? certNode
      : certNode && typeof (certNode as Record<string, unknown>)._text === 'string'
        ? (certNode as Record<string, string>)._text
        : certNode && typeof (certNode as Record<string, unknown>)['#text'] === 'string'
          ? (certNode as Record<string, string>)['#text']
          : null;
...
function buildSamlOptions(record: SamlConfigRecord): NodeSamlConfig {
...
function buildSamlInstance(record: SamlConfigRecord) {
  return new SAML(buildSamlOptions(record));
}

async function resolveOrgByDomain(email?: string | null) {
  if (!email || !email.includes('@')) return null;
  const domain = email.split('@')[1].toLowerCase();
  const result = await pool.query<{ org_id: string }>(`SELECT org_id FROM org_domains WHERE domain = $1`, [domain]);
...
export async function getOrgDomains(orgId: string) {
  const result = await pool.query<{ domain: string }>(`SELECT domain FROM org_domains WHERE org_id = $1 ORDER BY domain`, [orgId]);
  return result.rows.map((row) => row.domain);
}

export async function replaceOrgDomains(orgId: string, domains: string[]) {
  ...
  await withTransaction(async (client) => {
    ...
    for (const domain of unique) {
      await client.query(
        `INSERT INTO org_domains (org_id, domain) VALUES ($1, $2)
...
export async function getSamlConfig(orgId: string): Promise<SamlConfigRecord | null> {
  const result = await pool.query<SamlConfigRecord>(`SELECT * FROM saml_configs WHERE org_id = $1 LIMIT 1`, [orgId]);
  return result.rows[0] || null;
}

export async function getSamlConfigByIssuer(issuer: string): Promise<SamlConfigRecord | null> {
  const result = await pool.query<SamlConfigRecord>(
    `SELECT * FROM saml_configs WHERE idp_entity_id = $1 LIMIT 1`,
    [issuer],
  );
  return result.rows[0] || null;
}

export async function upsertSamlConfig(orgId: string, updates: Partial<SamlConfigRecord>) {
  const now = new Date().toISOString();
  await ensureOrg(orgId);
  const existing = await getSamlConfig(orgId);
  if (!existing) {
    const insert = await pool.query<SamlConfigRecord>(
      `INSERT INTO saml_configs (org_id, idp_metadata_url, idp_entity_id, idp_sso_url, idp_certificate, sp_entity_id, sp_acs_url, sp_metadata_url, enforce_sso, created_at, updated_at)
...
function mapProfile(profile: Profile): NormalizedProfile {
  ...
  const firstName =
    ...
    (attributes?.['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname'] as string) ||
    (profile as Record<string, unknown>)['givenName'] as string;
  const lastName =
    (attributes?.['lastName'] as string) ||
    (attributes?.['surname'] as string) ||
    (attributes?.['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname'] as string) ||
    (profile as Record<string, unknown>)['surname'] as string;
  const rolesRaw =
    (attributes?.['roles'] as string | string[] | undefined) ||
    (attributes?.['Role'] as string | string[] | undefined) ||
    (attributes?.['groups'] as string | string[] | undefined) ||
    [];
...
async function upsertUser(orgId: string, profile: NormalizedProfile) {
  ...
  const user = await withTransaction(async (client) => {
    const existing = await client.query(
      `SELECT * FROM users WHERE email = $1 AND (org_id IS NULL OR org_id = $2) LIMIT 1`,
      [email, orgId],
    );
    ...
    if (existing.rows[0]) {
      ...
      const updated = await client.query(
        `UPDATE users
         ...
         WHERE id = $1
         RETURNING *`,
        [current.id, orgId, profile.firstName ?? null, profile.lastName ?? null, role, now],
      );
      return updated.rows[0];
    }

    const inserted = await client.query(
      `INSERT INTO users (email, org_id, first_name, last_name, role, auth_provider, created_at, updated_at, last_login_at)
       VALUES ($1,$2,$3,$4,$5,'saml',$6,$6,$6)
       RETURNING *`,
      [email, orgId, profile.firstName ?? null, profile.lastName ?? null, role, now],
    );
...
function issueJwt(user: any, orgId: string) {
  const payload = {
    sub: user.id,
    email: user.email,
    orgId,
...
async function recordSession(orgId: string, userId: string, profile: NormalizedProfile, relayState?: string | null) {
  await pool.query(
    `INSERT INTO saml_sessions (org_id, user_id, name_id, session_index, relay_state, created_at)
     VALUES ($1,$2,$3,$4,$5,NOW())`,
    [orgId, userId, profile.nameId ?? null, profile.sessionIndex ?? null, relayState ?? null],
  );
}

export async function generateServiceProviderMetadata(orgId: string) {
  const configRow =
    (await getSamlConfig(orgId)) ||
...
export async function startSamlLogin(options: { orgId?: string | null; email?: string | null; relayState?: string | null; host?: string }) {
...
export async function handleAcs(params: { SAMLResponse?: string; RelayState?: string; orgId?: string | null }) {
  ...
  let configRow =
    (params.orgId && (await getSamlConfig(params.orgId))) ||
    (issuer ? await getSamlConfigByIssuer(issuer) : null);
  if (!configRow) {
    const fallback = await pool.query<SamlConfigRecord>(`SELECT * FROM saml_configs LIMIT 2`);
...
export async function handleIdpInitiated(params: { SAMLResponse?: string; RelayState?: string }) {
  return handleAcs(params);
}

export async function handleLogout(orgId?: string | null, email?: string | null) {
  if (email) {
...
export async function discoverSaml(email: string) {
...
export function parseAuthToken(authorization?: string, cookieHeader?: string) {
...
export function verifySessionToken(token?: string | null) {
  ...
  try {
    return jwt.verify(token, config.samlJwtSecret) as jwt.JwtPayload & { orgId?: string };
  } catch (error) {
    return null;
  }
}
