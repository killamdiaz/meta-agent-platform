...
import { ConnectorPackageInput, StoredConnectorPackage, isVersionGreater } from './types.js';
...
export class ConnectorService {
  ...
  constructor(private readonly store: ConnectorStore, options: ConnectorServiceOptions = {}) {
    this.registryPath = options.registryPath || config.connectorRegistryPath;
  }

  async saveDraft(tenantId: string, payload: ConnectorPackageInput) {
    const connectorId = (payload as any).id || randomUUID();
  ...
  async installConnector(tenantId: string, payload: ConnectorPackageInput) {
    const connectorId = (payload as any).id || randomUUID();
    const validated = validateConnectorPackage(payload);
  ...
  async publishConnector(tenantId: string, payload: ConnectorPackageInput, verified = false) {
  ...
  async installPublishedConnector(connectorId: string, tenantId: string) {
  ...
  private async persistToDisk(id: string, payload: ConnectorPackageInput) {
