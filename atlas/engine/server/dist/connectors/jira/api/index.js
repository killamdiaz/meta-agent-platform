...
router.get('/callback', async (req, res) => {
    try {
        ...
        let forgeUserId = resolveAccountId(req);
        try {
            const parsed = JSON.parse(decodeURIComponent(stateRaw));
            orgId = parsed.org_id ?? orgId;
            forgeUserId = parsed.forge_user_id ?? forgeUserId;
        }
        catch (err) {
        ...
        const redirectUri = buildRedirectUri(req);
        const tokenResp = await axios.post(ATLASSIAN_TOKEN_URL, {
...
router.post('/disconnect', async (req, res) => {
    try {
        ...
        await pool.query(`DELETE FROM forge_jira_tokens WHERE org_id = $1 AND account_id IS NOT DISTINCT FROM $2`, [orgId, accountId ?? null]);
        await pool.query(`UPDATE forge_integrations SET status = 'inactive', updated_at = NOW() WHERE org_id = $1 AND connector_type = 'jira'`, [orgId]);
...
router.get('/status', async (req, res) => {
    ...
    const { rows } = await pool.query(`SELECT jira_domain, cloud_id, expires_at, jira_user_id FROM forge_jira_tokens WHERE org_id = $1 AND account_id = $2 LIMIT 1`, [orgId, forgeUserId]);
    if (!rows[0]) {
        res.json({ status: 'inactive', data: {} });
        return;
    }
...
router.get('/projects', async (req, res) => {
    try {
        ...
        const forgeUserId = resolveAccountId(req);
        if (!orgId || !forgeUserId)
            return res.status(400).json({ message: 'org_id and forge_user_id required' });
...
router.get('/issues/:key', async (req, res) => {
    try {
        ...
        await ingestJiraIssue(issue, orgId, forgeUserId);
        const browseUrl = tokens?.jira_domain;
...
router.post('/issues/ingest', async (req, res) => {
    try {
        ...
        if (!orgId || !issue)
            return res.status(400).json({ message: 'org_id and issue required' });
