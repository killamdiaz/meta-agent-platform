...
export class JiraClient {
    constructor({ accessToken, cloudId, jiraDomain }) {
    ...
    static async fromUser(orgId, forgeUserId) {
        const token = await fetchUserJiraTokens(orgId, forgeUserId);
        if (!token || !token.cloud_id)
    ...
    static async fromTokens(tokens, orgId, forgeUserId) {
        let active = tokens;
        const expiresAt = tokens?.expires_at ? new Date(tokens.expires_at).getTime() : null;
        const shouldRefresh = orgId &&
            forgeUserId &&
            expiresAt !== null &&
    ...
    // Projects
    getProjects() {
        return this.request({ url: '/rest/api/3/project/search', method: 'GET' });
    }
    getProject(projectIdOrKey) {
        return this.request({ url: `/rest/api/3/project/${projectIdOrKey}`, method: 'GET' });
    }
    // Issues
    getAssignedIssues(accountId) {
        const jql = accountId
    ...
    searchIssues(params) {
        const clauses = [];
        if (params.jql)
            clauses.push(params.jql);
        if (params.query)
            clauses.push(`text ~ "${params.query}"`);
        if (params.status)
    ...
    createIssue(payload) {
        return this.request({ url: '/rest/api/3/issue', method: 'POST', data: payload });
    }
    updateIssue(issueKey, data) {
        return this.request({ url: `/rest/api/3/issue/${issueKey}`, method: 'PUT', data });
    }
    addComment(issueKey, body) {
        return this.request({ url: `/rest/api/3/issue/${issueKey}/comment`, method: 'POST', data: body });
    }
    addAttachment(issueKey, formData) {
        return this.request({
