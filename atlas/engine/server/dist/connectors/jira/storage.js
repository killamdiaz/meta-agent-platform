...
export async function upsertJiraToken(record) {
    await pool.query(`
...
export async function getJiraToken(orgId, forgeUserId) {
    const { rows } = await pool.query(`SELECT org_id, account_id as forge_user_id, jira_user_id, jira_domain, cloud_id, access_token, refresh_token, expires_at, scopes FROM forge_jira_tokens WHERE org_id = $1 AND account_id = $2 LIMIT 1`, [orgId, forgeUserId]);
    if (!rows[0])
        return null;
    const row = rows[0];
    return {
...
export async function refreshJiraToken(orgId, forgeUserId) {
...
export async function fetchUserJiraTokens(orgId, forgeUserId) {
