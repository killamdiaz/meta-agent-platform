...
function normalizeUsagePayload(payload, fallbackMeta) {
    const slackMeta = {
    ...
    const total = Number(payload.tokens_total ?? (Number.isFinite(prompt + completion) ? prompt + completion : 0)) || 0;
    const actions = Array.isArray(payload.actions_triggered) && payload.actions_triggered.length
...
export function appendUsageBlock(message, usage) {
...
export async function extractAndLogUsage(raw, fallbackMeta) {
    if (!raw)
        return null;
    ...
    try {
        ...
        let orgId = normalized.slack_metadata.org_id ??
            normalized.slack_metadata.team_id ??
            config.defaultOrgId ??
            null;
        let accountId = normalized.slack_metadata.account_id ??
        ...
        await pool.query(`INSERT INTO billing_usage
       ...
       VALUES (uuid_generate_v4(), 'slack', $1, $2, $3, $4, $5, $6, $7, $8, $9, NOW())`, [
        ...
        if (orgId) {
            await pool.query(`INSERT INTO forge_token_usage
