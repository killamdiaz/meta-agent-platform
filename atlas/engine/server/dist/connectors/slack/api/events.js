...
import { resolveOrgId, resolveAccountId, fetchSlackIntegration, fetchSlackIntegrationByTeamId, upsertSlackIntegration, } from './shared.js';
...
slackEventsRouter.post('/events', async (req, res) => {
    ...
    const botToken = integration?.data?.bot_token;
    ...
    try {
        if (event.type === 'app_mention') {
            await handleAppMention({ text: event.text, user: event.user, channel: event.channel, thread_ts: event.thread_ts ?? event.ts }, { orgId: resolvedOrgId, accountId: resolvedAccountId, slackClient, teamId: payload.team_id, eventType: 'app_mention' });
        }
        else if (event.type === 'message' && event.channel_type === 'im') {
            await handleDirectMessage({ text: event.text, user: event.user, channel: event.channel, thread_ts: event.thread_ts ?? event.ts }, { orgId: resolvedOrgId, accountId: resolvedAccountId, slackClient, teamId: payload.team_id, eventType: 'direct_message' });
        }
        else if (event.type === 'message') {
            await handleChannelMessage({ text: event.text, user: event.user, channel: event.channel, thread_ts: event.thread_ts ?? event.ts }, { orgId: resolvedOrgId, accountId: resolvedAccountId, slackClient, teamId: payload.team_id, eventType: 'channel_message' });
        }
