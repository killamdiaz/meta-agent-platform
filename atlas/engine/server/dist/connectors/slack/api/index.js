...
import { resolveOrgId, resolveAccountId, fetchSlackIntegration, fetchSlackIntegrationByTeamId, } from './shared.js';
...
export function buildSlackApiRouter() {
    ...
    router.post('/commands', express.urlencoded({ extended: true }), async (req, res) => {
        ...
        if (command.includes('kb'))
            await handleKbCommand(ctx);
        else if (command.includes('explain'))
            await handleExplainCommand(ctx);
        else if (command.includes('compare'))
            await handleCompareCommand(ctx);
        else if (command.includes('diagram'))
            await handleDiagramCommand(ctx);
        else if (command.includes('format'))
            await handleFormatCommand(ctx);
