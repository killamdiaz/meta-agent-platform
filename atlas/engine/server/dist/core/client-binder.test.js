...
describe('client-binder', () => {
    ...
    it('skips binding when credentials are missing and warns', async () => {
        ...
        assert.ok(warnings.some((entry) => entry.includes('Missing credentials')), 'expected warning about missing credentials');
    });
    it('logs skip for unknown agent types', async () => {
        ...
        assert.ok(logs.some((entry) => entry.includes('No binding found')), 'expected log indicating no binding found');
    });
});
