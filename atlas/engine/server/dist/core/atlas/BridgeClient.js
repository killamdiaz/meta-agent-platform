...
export class AtlasBridgeClient {
    ...
    async request(options) {
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt += 1) {
            ...
            try {
                ...
                if (response.ok) {
                    const result = (payload === undefined ? undefined : payload);
                    if (cacheTtl > 0 && descriptor) {
                ...
                const errorMessage = typeof payload === 'object' && payload && 'error' in payload
                    ? String(payload.error ?? 'Atlas Bridge request failed.')
    ...
    resolveCacheTtl(method, options, cacheable) {
    ...
    storeInCache(descriptor, value, ttlMs) {
    ...
    buildCacheDescriptor(method, options) {
        if (method !== 'GET') {
            return null;
        }
    ...
    serialiseBody(method, body, headers) {
...
export function generateSignature(agentId, token, secret) {
    if (!agentId)
        throw new Error('agentId is required to generate signature.');
    if (!token)
        throw new Error('token is required to generate signature.');
    if (!secret)
        throw new Error('secret is required to generate signature.');
