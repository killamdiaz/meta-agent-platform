...
router.post('/', async (req, res, next) => {
    try {
        ...
        const findAgent = (identifier) => {
            const lowered = identifier.toLowerCase();
            return (agents.find((agent) => agent.id === identifier) ||
                agents.find((agent) => agent.name.toLowerCase() === lowered));
        };
        const autoRoute = (text) => {
            const lowered = text.toLowerCase();
            return (agents.find((agent) => lowered.includes(agent.name.toLowerCase())) ||
                agents.find((agent) => agent.role && lowered.includes(agent.role.toLowerCase())) ||
                agents[0]);
        };
        const enqueue = async (agent, prompt) => {
