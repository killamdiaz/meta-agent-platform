...
const matchesKeyword = (agent, regex) => {
...
const findAgentByPrompt = (prompt, agents) => {
...
router.post('/tool-agents/run', async (req, res, next) => {
    try {
        ...
        if (payload.agentId) {
            ...
            target = agents.find((agent) => agent.id.toLowerCase() === lookup ||
        ...
        const mode = payload.mode && payload.mode !== 'auto'
            ? payload.mode
        ...
        const inferredLimit = payload.limit ??
...
router.get('/tool-agents/:agentId/logs', (req, res) => {
    ...
    const resolvedLimit = typeof limitParam === 'number' && Number.isFinite(limitParam) && !Number.isNaN(limitParam) ? limitParam : undefined;
