...
function mapMemoryStatus(memory) {
...
router.get('/graph', async (req, res, next) => {
    try {
        ...
        const [agentsResult, memoriesResult, tasksResult, integrationResult, crawlerDocsResult, jiraDocsResult] = await Promise.all([
            pool.query('SELECT id, name, status, created_at, updated_at FROM agents ORDER BY created_at ASC'),
          ...
          LIMIT 5000`, [userId]),
            pool.query(`SELECT id, agent_id, prompt, status, created_at
           FROM tasks
          ORDER BY created_at DESC
          LIMIT 200`),
            pool.query(`SELECT id, source_id, metadata, content, created_at
        ...
        const integrationNodes = integrationResult.rows.map((row) => {
            ...
            return {
                id: row.id,
                type: 'integration',
                label: meta.label || row.content || row.id,
                status: 'active',
                metadata: {
        ...
        const crawlerDocumentNodes = crawlerDocsResult.rows.map((row) => {
            const meta = row.metadata ?? {};
            const label = meta.title || row.content || row.source_id || row.id;
            return {
                id: `doc-${row.id}`,
                type: 'document',
        ...
        const memoryNodes = memoriesResult.rows.map((memory) => ({
            ...
            metadata: {
                ...
                createdBy: memory.metadata?.createdBy ?? memory.agent_id,
        ...
        const agentLinks = memoriesResult.rows.flatMap((memory) => agentsResult.rows.map((agent) => ({
            source: agent.id,
            target: memory.id,
        ...
        const integrationLinks = integrationNodes.flatMap((integration) => agentNodes.map((agent) => ({
            source: integration.id,
            target: agent.id,
            relation: 'shared',
            strength: 0.9
        })));
        const documentLinks = documentNodes.flatMap((doc) => agentNodes.map((agent) => ({
            source: doc.id,
            target: agent.id,
            relation: 'extends',
            strength: 0.4
