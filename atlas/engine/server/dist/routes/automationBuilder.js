...
import { automationPromptInterpreter } from '../services/AutomationPromptInterpreter.js';
const router = Router();
...
router.get('/events', (req, res) => {
    ...
    res.write('\n');
    const listeners = [];
    listeners.push(automationEventBus.onDrawer((event) => {
        if (event.sessionId !== sessionId)
            return;
        send('drawer', { isOpen: event.isOpen });
    }));
    listeners.push(automationEventBus.onPipeline((event) => {
        if (event.sessionId !== sessionId)
            return;
        send('pipeline', event.pipeline);
    }));
    listeners.push(automationEventBus.onNode((event) => {
        if (event.sessionId !== sessionId)
            return;
        send('node', event.node);
    }));
    listeners.push(automationEventBus.onEdge((event) => {
        if (event.sessionId !== sessionId)
            return;
        send('edge', event.edge);
    }));
    listeners.push(automationEventBus.onStatus((event) => {
        if (event.sessionId !== sessionId)
            return;
