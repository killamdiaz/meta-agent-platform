    ...
    try {
        const completion = await chatCompletion({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'Decompose the user query into 2-6 atomic, non-overlapping sub-queries focused on single ZIA/ZPA/ZCC/networking concepts (GRE, tunnel mode, identity, location attribution, ZEN hop tracing, precedence). Output ONLY JSON: {"subqueries":["..."]}. Remove duplicates and meaningless items.',
                },
        ...
        const subqueries = Array.isArray(parsed?.subqueries)
            ? parsed.subqueries.map((s) => String(s).trim()).filter(Boolean)
            : [];
...
async function multiTopicRetrieve(orgId, subqueries, sources, topK = 5) {
    const results = [];
...
function buildContextForLLM(originalQuestion, subqueries, retrieved) {
    const parts = [];
        ...
        if (parsed && typeof parsed.answer === 'string' && Array.isArray(parsed.citations)) {
            return {
                ...
                citations: parsed.citations
                    ...
                    .filter((c) => !Number.isNaN(c.paragraph_index)),
            };
        }
    }
    catch (err) {
...
export async function ragAnswer(options) {
    ...
    const uniqueCitations = Array.from(chunkMap.values());
