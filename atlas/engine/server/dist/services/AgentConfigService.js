...
function sanitizeField(field) {
    ...
    if (field.options && Array.isArray(field.options)) {
        sanitized.options = field.options.filter((entry) => typeof entry === 'string' && entry.trim().length > 0);
    }
    if (field.description)
        sanitized.description = field.description;
    if (field.placeholder)
        sanitized.placeholder = field.placeholder;
    if (field.tooltip)
        sanitized.tooltip = field.tooltip;
    if (field.defaultValue !== undefined)
        sanitized.defaultValue = field.defaultValue;
    return sanitized;
}
function fallbackTemplate(description) {
...
class AgentConfigService {
    async generateSchema(description, context = {}) {
    ...
    async upsertAgentConfig(agentId, payload) {
        if (!agentId)
            throw new Error('agentId is required');
        const sanitizedFields = payload.schema.map(sanitizeField);
        const client = await pool.connect();
        try {
            const { rows } = await client.query(`INSERT INTO agent_configs (agent_id, agent_type, summary, schema, config)
         ...
         DO UPDATE SET agent_type = EXCLUDED.agent_type,
                       ...
                       updated_at = NOW()
         RETURNING agent_id, agent_type, summary, schema, config`, [agentId, payload.agentType, payload.summary ?? null, JSON.stringify(sanitizedFields), JSON.stringify(payload.values ?? {})]);
            const row = rows[0];
            return {
    ...
    async getAgentConfig(agentId) {
        const { rows } = await pool.query(`SELECT agent_id, agent_type, summary, schema, config
         FROM agent_configs
        WHERE agent_id = $1`, [agentId]);
        const row = rows[0];
        if (!row)
            return null;
        const schema = Array.isArray(row.schema) ? row.schema.map(sanitizeField) : [];
        return {
            agentId: row.agent_id,
            agentType: row.agent_type,
    ...
    async deleteAgentConfig(agentId) {
