...
export class CalendarAgent extends AtlasModuleAgent {
    ...
    async handleOperationalMessage(message) {
        ...
        if (!payload.participants || payload.participants.length === 0) {
            ...
            await this.sendMessage(message.from, 'response', 'I asked EmailMonitoringAgent to confirm the participant list before scheduling.', { intent: 'calendar_pending_participants' });
            return;
        }
    ...
    async handleContextRequest(message) {
        ...
        const context = await this.fetchAtlas('/bridge-tasks', {
            limit: 5,
            tag: 'meeting',
            participant,
        });
        if (!context) {
            await this.sendMessage(message.from, 'response', 'Unable to fetch meeting follow-up tasks right now.', { intent: 'calendar_context_unavailable' });
            return;
        }
        await this.sendContextResponse(message.from, context, `Meeting follow-ups prepared for ${message.from}.`, { responder: this.id });
    }
    async handleContextResponse(message) {
        ...
        const participants = Array.isArray(payload.participants)
            ? payload.participants.filter((entry) => typeof entry === 'string' && entry.trim().length > 0)
            : pending.payload.participants;
        if (!participants || participants.length === 0) {
            await this.sendMessage(pending.requester, 'response', 'Still missing participant details; meeting not scheduled yet.', { intent: 'calendar_pending_participants' });
            return;
        }
    ...
    coerceMeeting(source) {
        if (!source || typeof source !== 'object' || Array.isArray(source)) {
            return null;
        }
        const record = source;
        const participants = Array.isArray(record.participants)
            ? record.participants.filter((entry) => typeof entry === 'string' && entry.trim().length > 0)
            : undefined;
        return {
    ...
    async scheduleMeeting(payload, requester, sourceMessageId) {
        const title = payload.title ?? 'Team Meeting';
        if (!payload.participants || payload.participants.length === 0) {
            await this.sendMessage(requester, 'response', 'Participant list still missing; meeting cannot be scheduled.', { intent: 'calendar_pending_participants' });
            return;
        }
        const meetLink = await this.postAtlas(CREATE_MEET_LINK, {
            title,
        ...
        if (!meetLink?.url) {
            await this.sendMessage(requester, 'response', 'I tried to create an Atlas meeting link but the bridge is unavailable.', { intent: 'calendar_creation_failed' });
            return;
        }
        const livekitToken = await this.postAtlas(GENERATE_LIVEKIT_TOKEN, {
            meetingUrl: meetLink.url,
            participants: payload.participants,
        });
        await this.sendMessage(requester, 'response', `Meeting scheduled!\nLink: ${meetLink.url}\nLiveKit token issued: ${Boolean(livekitToken?.token)}`, {
