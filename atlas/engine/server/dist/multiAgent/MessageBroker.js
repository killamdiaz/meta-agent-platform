...
export class MessageBroker extends EventEmitter {
    constructor() {
        ...
        this.agentAliasTopics = new Map();
        this.tokenUsageByAgent = new Map();
        this.totalTokens = 0;
    }
    publish(payload) {
    ...
    updateAgent(agentId, patch) {
        ...
        if (!current)
            return;
        const nextConnections = patch.connections !== undefined
    ...
    syncTopicSubscriptions(agentId) {
        const descriptor = this.agents.get(agentId);
        const previous = this.agentTopics.get(agentId) ?? new Set();
        if (!descriptor) {
        ...
        const aliases = this.agentAliasTopics.get(agentId) ?? new Set();
        const next = new Set([agentId, ...descriptor.connections]);
    ...
    registerTopicAlias(agentId, topic) {
        if (!topic)
            return;
        const descriptor = this.agents.get(agentId);
        if (!descriptor) {
    ...
    unregisterTopicAlias(agentId, topic) {
        if (!topic)
            return;
    ...
    trackTokenUsage(message) {
        const rawValue = message.metadata?.tokens;
