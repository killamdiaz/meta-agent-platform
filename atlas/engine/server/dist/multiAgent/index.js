...
const metaAgentIdEnv = process.env.META_AGENT_ID ?? process.env.NEXT_PUBLIC_META_AGENT_ID ?? process.env.VITE_META_AGENT_ID;
const metaAgentSecretEnv = process.env.META_AGENT_SECRET;
const metaAgentTokenEnv = process.env.META_AGENT_JWT ?? process.env.META_AGENT_TOKEN ?? process.env.ATLAS_BRIDGE_TOKEN ?? '';
const notificationConfig = metaAgentIdEnv && metaAgentSecretEnv && metaAgentTokenEnv.trim().length > 0
    ? {
...
if (!notificationConfig) {
    if (metaAgentIdEnv && metaAgentSecretEnv) {
        console.warn('[meta-cortex-bus] Atlas Bridge notifications disabled; set META_AGENT_JWT (or META_AGENT_TOKEN/ATLAS_BRIDGE_TOKEN) to enable.');
    }
    else {
        console.warn('[meta-cortex-bus] Missing META_AGENT_ID/META_AGENT_SECRET; Atlas Bridge notifications are disabled.');
    }
}
