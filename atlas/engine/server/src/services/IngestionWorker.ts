...
async function fetchNextQueuedJob(): Promise<ImportJobRow | null> {
...
async function updateJobStatus(
  ...
  const values: unknown[] = [id, status, progress, JSON.stringify(metadata ?? {})];
...
async function processJob(job: ImportJobRow) {
  if (!job.source) {
  ...
  try {
    ...
    console.log(
      `[ingestion-worker] crawl finished for job ${job.id} source=${job.source} discovered=${discovered} pages=${totalPages}`,
    );
    await updateJobStatus(
      job.id,
      'processing',
      totalPages === 0 ? 10 : Math.min(30, Math.floor((0 / totalEstimated) * 90) + 10),
      {
        current_url: job.source,
    ...
    for (const page of pages) {
      ...
      console.log(
        `[ingestion-worker] job=${job.id} page ${processed + 1}/${totalForProgress} url=${page.url}`,
      );
      await updateJobStatus(
        job.id,
        'processing',
    ...
    await updateJobStatus(job.id, 'failed', 0, { error: (error as Error).message });
  }
}
