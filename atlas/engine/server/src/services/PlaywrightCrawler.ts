import { chromium, BrowserContext, Page, Route, Browser } from 'playwright-core';
import { URL } from 'node:url';
import * as cheerio from 'cheerio';

type CrawlResult = { url: string; title: string; content: string };
type ProgressHandler = (url: string) => Promise<void> | void;
...
function shouldCrawl(url: string, baseDomain: string, allowedPaths: Set<string>): boolean {
  try {
    const parsed = new URL(url);
    ...
    if (skipPatterns.some((re) => re.test(parsed.pathname))) return false;
...
export async function crawlSite(
  ...
  onStats?: (stats: { visited: number; queued: number; discovered: number }) => Promise<void> | void,
): Promise<CrawlResult[]> {
  ...
  const allowed = new Set<string>([basePath]);
  additionalPaths
    .filter((p) => p)
    .forEach((p) => {
  ...
  const queue: string[] = [startUrl, ...additionalPaths.map((p) => new URL(p, startUrl).toString())];
  ...
  while (queue.length && (maxPages === -1 || results.length < maxPages)) {
    ...
    if (!shouldCrawl(normalized, baseDomain, allowed)) continue;

    if (onProgress) await onProgress(normalized);
    ...
    try {
      ...
      const title =
        (soup('title').text() || soup('h1').text() || '').trim();
      ...
      const links = await page.$$eval<string[]>('a[href]', (els) => els.map((el) => (el as HTMLAnchorElement).href));
      links.forEach((link) => {
