...
router.put('/auth/saml/config/:orgId', async (req, res, next) => {
  try {
    ...
    const domains = Array.isArray(req.body?.domains) ? (req.body.domains as string[]) : null;
...
router.get('/auth/saml/session', (req, res) => {
  ...
  res.json({
    user: {
      ...
      provider: 'saml',
      first_name: (payload as any).firstName,
      last_name: (payload as any).lastName,
      user_metadata: { org_id: payload.orgId, role: (payload as any).role },
