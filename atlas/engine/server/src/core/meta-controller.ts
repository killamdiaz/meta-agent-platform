...
function buildFallbackSchema(agentName: string): AgentSchema {
  return {
...
function sanitiseSchema(agentName: string, payload: unknown): AgentSchema {
...
export async function generateDynamicAgentSchema(agentName: string): Promise<AgentSchema> {
  const prompt = [
  ...
  try {
    const content = await routeMessage({
      prompt,
      context:
        'Respond with valid JSON only. Include fields name, description, inputs, outputs, capabilities.',
