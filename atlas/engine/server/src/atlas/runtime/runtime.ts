...
import { StoredConnectorPackage, RuntimeExecutionResult, RuntimeAuthContext } from '../core/types.js';
import { getJsonPath } from './json-path.js';
import { TransformRunner } from './transform-runner.js';

type RetryableStatus = 429 | 500 | 502 | 503;
const RETRYABLE_STATUSES: RetryableStatus[] = [429, 500, 502, 503];

function applyTemplate(input: any, params: Record<string, any>): any {
...
function mapResponse(mapping: Record<string, string>, payload: any) {
...
export class UniversalConnectorRuntime {
  ...
  private transformRunner = new TransformRunner();

  constructor(private readonly vault: CredentialVault, private readonly client: AxiosInstance = axios.create()) {}

  async executeAction(params: {
    connector: StoredConnectorPackage;
    action: string;
    ...
    for (let i = 0; i < attempts; i += 1) {
      try {
        ...
        lastError = err;
        const status = err?.response?.status as RetryableStatus | undefined;
  ...
  private buildRequest(action: any, input: Record<string, any>, auth?: RuntimeAuthContext): AxiosRequestConfig {
    const params = input || {};
    ...
    const request: AxiosRequestConfig = {
      ...
      data: body,
      timeout: 15_000,
      validateStatus: (status) => status < 500 || RETRYABLE_STATUSES.includes(status as RetryableStatus),
    };

    return request;
  }

  private buildUrl(pathValue: string, baseUrl?: string) {
  ...
  private async resolveAuth(
    ...
    tenantId: string,
    authKey?: string,
  ): Promise<RuntimeAuthContext | undefined> {
  ...
  private async refreshToken(connector: StoredConnectorPackage, refreshToken: string) {
  ...
  private ensureCircuit(connectorId: string) {
  ...
  private recordFailure(connectorId: string) {
  ...
  private recordSuccess(connectorId: string) {
