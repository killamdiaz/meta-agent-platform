...
import { ConnectorPackageInput } from './types.js';

export function validateConnectorPackage(input: ConnectorPackageInput) {
  ...
  validateNoSecretsInFiles({
    ...
    ...Object.fromEntries(
      Object.entries(parsed.triggers).map(([k, v]) => [`triggers/${k}.json`, JSON.stringify(v)]),
    ),
...
export function ensureNoInlineSecrets(payload: unknown) {
